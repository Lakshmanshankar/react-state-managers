...
import { Todo, Todostatus, defaultTodo } from "@/Todos/shared";
...
import { useZustandTodos } from "@/Todos/Zustand/Todo";

export default function TodoUI() {
  console.log("[ZUSTAND] INPUT RENDERED");
...
export const ZustandTable = React.memo(function RenderTodoTables() {
  const { todos, changeStatus, deleteTodo } = useZustandTodos();
  console.log("[ZUSTAND] TABLE RENDERED");
  const handleChange = (
...
export function CheckZustand() {
  console.log("[ZUSTAND] CHECK RENDERED");
  const { addTodo } = useZustandTodos()
  return (
    <div className="w-full grid place-content-center text-2xl py-10 ">
      ...
      <Button className="px-10 w-fit" onClick={() => {
        addTodo({ index: Math.random() * 120, name: 'Some Todo', status: 'In Progress' })
